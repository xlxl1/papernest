...
2. **真去量了一次**，结果是负的：三臂配对实验（QASPER 88 题、等预算、0 token）
   ...
   | C 换成章节块 | 0.2045 | **0.1136** | 17/88 | 0.331 |

   所以默认单元仍是 `page`。**这些用例的作用是：把负结果钉住，别让人凭直觉把开关打开。**
...
class NegativeResultIsPinned(unittest.TestCase):
    ...
    def test_default_context_unit_is_page(self):
        self.assertEqual(
            ...
            "换成 chunk 是实测负结果（证据召回 0.2614→0.2045，保留空白口径 →0.1136）。"
            "chunks 表的文本是 pages 的有损再推导，会打断机械回取校验。"
            "要改默认值先跑 `python cli.py qasper ctxab` 拿出新数字。")
