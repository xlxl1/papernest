...
> 设计与方案文档不随代码仓库分发；需要公开展示时另行放入 `docs/` 再补链接。
...
## 当前状态：核心能力主体完成 · 写作流水线已落地 · 本地入库与引文网络已落地

- [x] **Agent 核心**：Router/Planner（确定性意图路由）· Tool registry（5 工具）· 有界状态机执行 · `/api/agent/run` · 执行轨迹落库可回放
- [x] **证据与评测**：页级 chunk（PyMuPDF 按页入库）· evidence quote（支撑证据句 + 机械回取校验）· claim verification（L2 卡片逐条 + 综述逐句）· **自建评测集**（当前 82 条：检索 32 关键词串 + 32 自然语言形态 · QA 10 · 引用 8） · **三指标评测脚本** · **QASPER 公开基准接入**（paper_hit@k + 证据句 Recall 两层指标）
- [x] **工程化**：异步任务（SQLite 任务表）· SSE 实时进度（含心跳保活）· token / latency / cost 逐次落账 · 步骤级超时（协作式取消）+ 瞬态故障重试（4 次抖动退避）· 离线 Demo（`cli.py demo`）· 流式对话（`/api/chat/stream`）· **RRF 混合检索 + 口径消融** · **规模实验命令**（`cli.py scale`，吞吐/去重/p50/p95）· **换提供方向量零成本迁移**
- [x] **包装与展示**：生成综述入口 · 执行时间线 UI（Agent 页 + 历史回放）· **写作台（润色 + 评审）** · **文献汇报 PPT** · Docker 一键启动 · 架构图（下）· 数字一览（文末）
- [x] **多 Agent 写作流水线**：选题 / 大纲 / 撰写 / 文献 / 润色五 Agent 确定性流水线（见下节）
- [x] **把「个人文献库」补完整**（见下节）：**本地 PDF 上传入库** · **BibTeX / RIS / Zotero CSV 导入** ·
      **引文网络（阅读缺口 + 共被引推荐）** · **服务端多轮对话记忆** · 迭代检索（PRF 补位，
      当前库重测 Recall@15 0.8125→0.8281，**+0.0156 在噪声内，方向为正但不显著**）·
      课题工作区网页端
- [x] **发现与阅读**：**L2 全文进全库检索**（QASPER hit@1 0.410→0.467）· **修好 QASPER 基准链路**
      （下载源全部 404 + 数据格式已变，此前是坏的）· **原文按页渲染，证据链在界面闭环** ·
      arXiv 新论文订阅（批内 IDF + 广度折扣）· 结构化对比矩阵（页码逐条校验）·
      PDF 版面结构解析（章节切分 + 参考文献抽取 → 反哺引文图）· **491 条自动化测试**
- [x] **收尾与生态**：**迭代问答闭环接线**（`ask --deep` / `/api/answer/deep`，此前实现了但没有任何入口）·
      综述与写作选题切到迭代检索口径 · 对话历史 UI（回放/继续/删除）· **Obsidian 笔记导出**（citekey 双链）·
      Ollama/llama.cpp 本地模型配置样例
- [x] **RCS 增强问答**：LLM 重排 + 逐篇定向摘要 + **依据句逐页机械回取校验**（比 PaperQA2 多的一层）·
      A/B 实测（gold 引用覆盖 0.818→1.0；无证据率无可靠改善，如实记录）· **533 条自动化测试**
      （RCS 的 22 条用例大半在测失败模式而非 happy path）
- [x] **文档处理深度**：**按章节切分检索单元**（等上下文预算下证据召回是按页切的
...
- [x] **向量层收口**：Milvus 后端抽象与同步实现（`PAPERNEST_VECTOR_BACKEND` 默认 `milvus`，
      numpy 一行可切回、Chroma 可选）· SQLite `vectors` 表为唯一真相来源，Milvus 只是派生索引：
      写路径先落 SQLite 再同步，`cli.py vec rebuild` 随时可从 SQLite 全量重放，陈旧命中计入 `last_stale_hits` ·
      连不上**不静默降级**（抛 `VectorStoreUnavailable`，显式设 `PAPERNEST_VECTOR_FALLBACK=1` 才退回 numpy 并留痕）·
...
- [x] **审计与收口**：全仓生产化审计（14 维度 · 每条发现两个独立视角对抗核验）后修掉三条：
...
- [ ] Demo 视频录制（材料就绪，待录）
- [x] **补齐全库向量**（443 篇 / 约 3.5k 条嵌入）：覆盖 20.7% → **100%**，
      线上口径全部重测（见「评测」一节的配对对比）；`chunk` 向量 784 条**零丢失**，
      是修复 ① 的生产验证
- [ ] 补难题进评测集：现有题目分辨不出「向量覆盖 20.7% vs 100%」（各指标只变 1 题）
- [x] **按审计清单逐条修复**（2026-09-07/08，每条都在真库上复现 + 回归测试先红后绿）：
      **正确性 12 条**——`rcs` 定向摘要只喂前 3 页（证据覆盖 10.2%→39.0%，p=1.9e-08；**显著的一半是预算不是选页**）·
      `deepsearch` 补检索闸门取错集合（迭代检索结构上无法引入新文献）·
      `rag._carry_over` 跨轮回指查错了表 · `chat.is_followup` 指代标记短路检索信号 ·
      `agent.build_plan` 裸子串路由（贴标题问被引数 503/503 劫持 → 1/503）·
      `matrix` 的 `verified` 构造性恒真（自证率 0.983 → 真实的 0.25）·
      参考文献段被当正文进检索（真库 13.9% 的可检索文本）· `structure` 漏检子标题
      （TOC recall 76.7%→80.4%，**误检零新增**）· `sectiontree.build_all` 不用结构化章节
      （假节 4.77%→0.34%、溯源标签正确率 46%→100%）· `_pick_pages` 跨语言
      （中文提问 fallback 率 98.5%，接上向量命中的页）· chunk 长度上限只在一条路生效 ·
      预算 best-fit（**核实为负结果**，只修可观测性）·
      **数据完整性 6 条**——DOI 正则抄了 4 份而漏了导出层（真库 100/242 篇导出即坏）·
      arXiv error-feed 采集路径没挡 · `document_sources` 可空主键让 upsert 永不触发 ·
      `query_cache` 无 TTL · `bibimport` 无熔断（S2 挂掉时 500 条导出跑 21 小时）·
      引用编号超两位数静默失效 ·
      **前端 3 条**——作者字段 XSS · **原有的引号补丁本身是坏的**（实测 onclick 编译不出来）·
      流式出错时 UI 永远停在「正在组织回答…」 ·
      **运维 6 条 + 评测可信度 6 条**（见下文各处口径说明）。
- [x] **三条「大件」按部署设想（本机单用户）重判**：不做多租户（改为把边界钉住）·
      不做限流、改做每日 token 闸门（`papernest/budget.py`，防失控烧额度）·
      日志做成一个轮转文件 + traceback（`papernest/log.py`），并给 4 个把异常吞成
      HTTP 200 的端点补上留痕。全仓 225 个 `except` 里 161 个不记录，**只动了信息会
      彻底丢失的那一类**。
...
7. **STORM 视角引导的综述规划**（高成本高差异化）。

## 把「个人文献库」补完整

此前能搜、能读、能写，但有三个洞让它称不上「个人」文献库：**手上的 PDF 进不来**、
**Zotero 里攒的几百篇搬不进来**、**只能管已有的、发现不了该读而没读的**。这一节补的就是这三件事。
...
**质量闭环**：每节初稿 → 结构化评审（复用既有 review：总评 / 1-10 分 / major-minor
...
（在 503 篇库上重测，纯 FTS 底座、关键词串形态 32 条）
...
**查询预处理：用户真会打的输入不能把检索打废**。原来的切词只剥双引号，
...
⚠️ **这组数字已作废，不要引用**：本文档旧版记 399.9ms → 16.1ms（25 倍），`milvusstore.py` 的注释记 380.3ms → 8.0ms（47 倍），两处互相矛盾，而仓库里没有任何 bench 脚本能裁决（tests/ 下零行 Milvus 计时代码）。**代码里那组已经删掉了**——一个既无法复现、又与文档打架的「实测值」比没有数字更糟，它会被当成依据引用。选 Bounded 的理由是**机制性**的：Strong 要等写入时间戳同步才开始搜，而文献检索是「写一次、读很多次」——这个理由不依赖任何具体毫秒数。要重新给数字，得先有能在真实 Milvus 上跑、并把版本 / 数据量 / 索引类型 / 网络一起记下来的 bench。
...
> ⚠️ **这个 71% 的归因要更正**（2026-09-07 复核）。此前把它读成「模型过滤激进」，
> 但当时 `_body_text` 只喂**前 3 页 + 3000 字符**，QASPER 实测只有 **10.2%** 的题
> 有 gold 证据进得了摘要模型眼前——它判「不相关」时，很多情况下输入里根本没有证据。
> 修完（挑页 + 预算 6000）证据覆盖升到 **39.0%**（b=37 c=3，p=1.9e-08）。
> 上面这组 71% / 1.5 篇 / gold 覆盖 1.0 是**修复前**测的，**合入后需要重跑**；
> 在重跑之前不要拿它论证「模型过滤有多激进」。
> 另外：这条修复里**显著的一半是预算不是选页**——「挑页 vs 前 3 页」在
> 3000/6000/8000 三档预算上分别是 p=1.0 / 0.108 / 0.312，一档都不显著。
...
2. **后来真去量了一次，结论与它相反**——见下面「上下文组装：一条负结果」。
3. **当时的评测口径对一个已知缺陷不敏感**：`qasper` 的 gold 匹配先删掉全部空白再做子串包含，
   ...
   **该缺陷已修复**（见下节），但**存量 chunks 尚未重切**。

诚实的说法是「**按章节切在检索排序上有效（块级权重表可复现）；
而「按章节块组装上下文」这一步实测为负收益，已如实记录并保持默认关闭**」。

### 换 embedding 模型：三条只有真跑才会撞上的坑

免费额度耗尽后换到 `qwen3.7-text-embedding-flash`。过程中撞到三件事，都值得记：

**① 新旧嵌入空间几乎正交，而维度相同。** 用库里已存的旧向量比对（不需要旧模型还有额度）：
中位数余弦 **0.0240**，两个模型都是 1024 维。**维度检查完全挡不住**——
如果直接换模型不重建，新查询向量与旧文档向量算出来的余弦是没有意义的数。
挡住它的是 `vectors.model` 列隔离：换模型后矩阵为空，如实报
「向量索引为空（EMBED_MODEL=… 匹配不到任何向量，检查模型名或先跑 cli.py embed）」，
健康探针同步标红。**这正是当年那次「模型名填成显示名 → 静默退化成纯 FTS、代价 0.27 Recall」
事故的防线，第一次真正派上用场。**

**② `embed_texts` 一次瞬态超时就让整条 `cli.py embed` 挂掉。** 这条命令要跑几百次调用，
没有重试等于「跑到哪算哪」。已补退避重试，口径与 `llm.chat` 一致：
400（模型名错、额度耗尽）/401/403 立即失败，429/5xx 与网络抖动才退避。

**③ 端点对超限输入是挂起，不是返回 400。** 实测同一篇摘要：
...
所以不是固定字符阈值——token/字符比随语言差好几倍（英文约 1:4、中文常常 1:1），
固定上限没法安全。改成**自适应截断**：超时就把输入减半重试（6000→…→400），
成功后如实记录生效上限（`LAST_TRUNCATION`），不让「这条向量只覆盖半篇」悄悄发生。
并且**把学到的上限跨批复用**——否则每批都要重新从 6000 往下踩一遍超时，
实测就是这一条把 804 条 chunk 从「16 条跑十几分钟」变成「788 条 207 秒」。

（我一度以为是三连换行触发的——二分边界正好落在 `


CONCLUSIONS` 上，
直接构造各种换行组合验证后否掉了。**边界巧合不是根因。**）
...
**存量 chunks 已重切完毕**：26 份 PDF 从旧会话的临时目录归位到
...
> 修查询预处理之前，自然语言形态是 0.5312，比关键词串低 0.224（p=0.0071）。
>
> **线上口径**（向量 + FTS）Recall@5 = **0.9427**、@10 = 0.9740（补齐向量后重测）。
> **这个数字有一个必须一起说的前提**：
>
> 1. **该评测在 numpy 后端下测得，但两个后端已经对齐过。** 检索热路径现在走
>    `vectorstore.get_store()`，同一条查询向量在真库（4816 条向量、1024 维）上分别问
>    numpy 与 Milvus，返回的 top-5 论文、命中的 kind 与分数**逐位相同**；Milvus 后端
>    （HNSW）与精确检索的 top-5 一致性另有实测 30/30。所以这个数字不因换后端而变；
>    真要换后端重测出差异，以重测报告为准。
...
在全库检索里等于不存在——L2 最大的浪费。现在正文按章节切成块、单独进 `chunks_fts`
（不是塞进 papers_fts 的第四列：命中要能定位到具体哪一块，才对得上「证据带页码/章节号」
的口径），以单独的权重参与 RRF 融合。

> 口径提醒：**别把这个权重读成「影响很小」**。RRF 的分数是 `weight / (rrf_k + rank + 1)`，
> rrf_k=60 远大于候选深度，主路的动态范围被压得很扁——默认 `top_k=5`（候选深度 15）下，
> 权重 1.0 的主路从第 1 名到第 15 名总共只值 0.00306，而块级路第 1 名就贡献
> 0.2/61 = 0.00328。也就是说块级命中一次足以抹平主路里的任意名次差。
> 这个数随 `top_k` 变（top_k=8 时主路跨度反超）。下面这张表是在**默认档位**上实测的，
> 仍然有效；不成立的是「0.2 所以影响小」这个推理。

> **自建集的成分要说清楚**（2026-09-07 实测，判据 = 查询词面有多少落在 gold 标题里）：
> keyword 形态 32 条里 **10 条（31%）的重叠 ≥80%**，其中 5 条查询词是 gold 标题的真子集
> ——这部分实际上测的是「能不能把标题查回来」，不是检索质量；
> natural 形态 32 条里 **0 条** ≥80%。所以**两个形态要一起报**：只报 keyword 会高估。
> 成分由 `tests/test_eval_set_composition.py` 钉住（高重叠条数只许减不许增）。

权重是**测出来的**。在 503 篇库上重测（QASPER 88 题 · 自建 32 条，纯 FTS 底座）：
...
flowchart LR
    ...
    API --> WP[写作流水线 pipeline<br/>选题→大纲→逐节撰写⇄文献白名单→润色<br/>两人工检查点 · 节级内容寻址缓存<br/>机械终检：引用核验率 / 悬空编号]
...
- **访问口令：从「可选」改成「暴露时强制」**。原来它是可选项，而唯一的部署方式
...
「写作流水线已落地」的说法当时是打了折的。现在 `tests/test_write_online.py` 用假模型把
...
>
> **向量覆盖率已补齐到 503/503（100%）**（`cli.py embed`，443 篇、约 3.5k 条嵌入）。
...
| 数字 | 值 | 复现方式 |
|---|---|---|
| 代码 / 接口 / 测试 | 21.5k 行 Python（56 个文件）· 83 个 API 端点 · **1306 条离线确定性测试全绿（约 190s）** | `$env:PYTHONPATH='.'; python -m unittest discover -s tests -t tests` |
...
| **向量覆盖率** | **503/503（100%）** 篇能被语义检索命中 | `python cli.py health`（四项全绿；低于 95% 覆盖报失败） |
| 嵌入模型 | `qwen3.7-text-embedding-flash`（1024 维，4816 条向量） | 换用后与旧模型**空间几乎正交**（余弦 0.0240），已全量重建 |
| 自建集 Recall@5（线上口径 · 关键词串形态） | **0.9583** | `python cli.py eval --retrieval auto --k 5` |
| 自建集 Recall@5（线上口径 · **自然语言提问形态**） | **0.9635**（略高于关键词串，形态差距 −0.0052） | 同上，看 `retrieval_metric_natural` |
| QASPER paper_hit@5（线上口径） | 0.7500 | `python cli.py qasper eval --n-papers 30` |
...
| Milvus 一致性档位 | ⚠️ **数字已作废**：两处记录互相矛盾（25 倍 vs 47 倍）且都无复现脚本，代码里那组已删除。默认 Bounded 的理由是机制性的（Strong 要等写入时间戳同步），不依赖具体毫秒数。 | — |

面向求职的说明材料放在代码仓库之外，不随代码分发。

---

*PaperNest · 第 0 步数据源验证通过后动工；评测与演示材料随开发推进更新。
多 Agent 写作系统已落地：选题/大纲/撰写/文献/润色五 Agent 流水线、
引用白名单制、节级内容寻址缓存、机械终检与 md/docx 导出。*
