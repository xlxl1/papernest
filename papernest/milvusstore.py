...
# 一致性级别。**默认 Bounded 是量出来的**：真实 Milvus v2.5.10 上同一批查询，
# Strong p50 380.3ms、Bounded 8.0ms、Eventually 7.6ms——差 47 倍。
# Strong 会等写入时间戳完全同步才搜，对「写一次、读很多次」的文献检索是错误的默认值：
# 一次入库之后要付上千次查询的代价。Bounded 允许读落后写几秒，对本场景无影响
# （新入库的论文晚几秒才能被检索到，用户根本感知不到）。
